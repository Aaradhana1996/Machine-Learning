...
cosmetics_1R <- OneR(PrimaryCategory ~ ., data = cosmetics_train)
...
library(gmodels)
answerTable <- CrossTable(cosmetics_pred, cosmetics_test$PrimaryCategory,
                          prop.chisq = FALSE, prop.r = FALSE, prop.t = FALSE,
                          dnn = c('predicted PrimaryCategory', 'actual PrimaryCategory'))
...
cosmetics_JRip <- JRip(PrimaryCategory ~ ., data = cosmetics_train)
cosmetics_JRip
summary(cosmetics_JRip)

cosmetics_pred_jrip <- predict(cosmetics_JRip, cosmetics_test)

answerTablejrip <- CrossTable(cosmetics_pred_jrip, cosmetics_test$PrimaryCategory,
                              prop.chisq = FALSE, prop.r = FALSE, prop.t = FALSE,
                              dnn = c('predicted PrimaryCategory', 'actual PrimaryCategory'))
